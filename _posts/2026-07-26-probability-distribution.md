...
​	*考试的时候会问你二项分布中某一件事最可能成功的次数，即二项分布的众数：*

​	若$(n + 1)p$是整数，则最可能次数有两个：$k_0 = (n + 1)p$和$k_0 = (n + 1)p - 1$。若$(n + 1)p$不是整数，则最可能次数为$k_0 = \lfloor (n + 1)p \rfloor$
...
​	设正态分布$X_1 \sim N(\mu_1 , \sigma_1^2)$，$X_2 \sim N(\mu_2 , \sigma_2^2)$相互独立，则它们的线性组合依然服从正态分布：
$$
